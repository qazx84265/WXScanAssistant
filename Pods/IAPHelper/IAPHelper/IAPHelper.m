...
@property (nonatomic, copy) IAPBuyProductBeginningBlock buyBeginningBlock;
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error {
    NSLog(@"----------->>>>>>>>>>>> product request err: %@", [error localizedDescription]);

    if(_requestProductsBlock) {
        _requestProductsBlock (nil,nil, error);
    }
}
    ...
    for (SKPaymentTransaction *transaction in transactions)
    {
        switch (transaction.transactionState)
        {
            case SKPaymentTransactionStatePurchasing:
                if (self.buyBeginningBlock) {
                    self.buyBeginningBlock();
                }
                break;
...
- (void)buyProduct:(SKProduct *)productIdentifier onCompletion:(IAPbuyProductCompleteResponseBlock)completion {
    [self buyProduct:productIdentifier onBegin:nil onCompletion:completion];
}

- (void)buyProduct:(SKProduct *)productIdentifier onBegin:(IAPBuyProductBeginningBlock)beginning onCompletion:(IAPbuyProductCompleteResponseBlock)completion {
    self.buyProductCompleteBlock = completion;
    self.buyBeginningBlock = beginning;
