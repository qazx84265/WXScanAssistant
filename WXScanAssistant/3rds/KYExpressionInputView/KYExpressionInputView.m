...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        //subview
        self.containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 5, frame.size.width, frame.size.height - 44)];
...
- (void)configExpressionContainerViewForIndex:(NSUInteger)index{
    ...
    if (!view) {
        ...
        if ([items.firstObject expressionDataType] == kExpressionDataTypeEmoji) {
...
- (NSArray *)configItemsIfEmoji:(NSArray *)items row:(NSUInteger)row column:(NSUInteger)column {
    if ([[items firstObject] expressionDataType] == kExpressionDataTypeEmoji&&row*column>1) {
        //计算位置，插入删除按钮
        BOOL stop = NO;
