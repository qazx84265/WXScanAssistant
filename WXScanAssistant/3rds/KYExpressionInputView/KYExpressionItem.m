...
@property (nonatomic, assign) KYExpressionDataType expressionDataType;
...
+ (instancetype)deleteItem {
    ...
    item.expressionDataType = kExpressionDataTypeDelete;
    return item;
}
...
- (instancetype)initWithImage:(UIImage *)image url:(NSString *)url {
    ...
    if (self = [super init]) {
        ...
        if (image.images.count>1) {
            _expressionDataType = kExpressionDataTypeGif;
        }
        else{
            _expressionDataType = kExpressionDataTypeImage;
        }
    }
    return self;
}
...
- (instancetype)initWithEmoji:(NSString *)emoji {
    if (self = [super init]) {
        _text = emoji.copy;
        _expressionDataType = kExpressionDataTypeEmoji;
    }
    return self;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:@(self.expressionDataType) forKey:NSStringFromSelector(@selector(dataType))];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super init]) {
        if (aDecoder) {
            ...
            _expressionDataType = [[aDecoder decodeObjectForKey:NSStringFromSelector(@selector(dataType))] unsignedIntegerValue];
...
- (NSData *)imageData {
    if (!_imageData) {
        if (self.expressionDataType == kExpressionDataTypeGif) {
