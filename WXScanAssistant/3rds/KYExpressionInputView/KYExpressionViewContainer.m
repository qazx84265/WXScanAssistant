...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _textLabel.font = [UIFont systemFontOfSize:32];
...
- (void)layoutSubviews {
    ...
    switch ([self.expressionItem expressionDataType]) {
...
- (void)setExpressionItem:(id<KYExpressionData>)expressionItem {
    if (_expressionItem != expressionItem) {
        ...
        switch ([expressionItem expressionDataType]) {
            case kExpressionDataTypeEmoji:
...
- (void)initUI {
    ...
    //pageControl
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame) - 23, self.bounds.size.width, 20)];
